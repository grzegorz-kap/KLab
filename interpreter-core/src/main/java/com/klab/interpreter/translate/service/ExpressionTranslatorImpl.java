...
public class ExpressionTranslatorImpl implements ExpressionTranslator {
	...
	private boolean getAnsProperty() {
		return BooleanUtils
				.isNotFalse(translateContext.getExpression().getProperty(Expression.ANS_PROPERTY_KEY, Boolean.class));
	}

	private boolean getPrintProperty() {
		return BooleanUtils
				.isTrue(translateContext.getExpression().getProperty(Expression.PRINT_PROPERTY_KEY, Boolean.class));
	}
